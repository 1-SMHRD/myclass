...
public class WDHMyBatisDAO {
	...
	//리뷰입력
	public void reviewWrite(PlaceReviewVO rvo) {
		SqlSession session = sqlSessionFactory.openSession();
		session.insert("reviewWrite",rvo);
		session.commit(); // 인서트는 commit 필수임
		session.close();
	}
	//리뷰가져오기
	public List<PlaceReviewVO> reviewlist(PlaceReviewVO vo) {
		SqlSession session = sqlSessionFactory.openSession();
		List<PlaceReviewVO> rvo = session.selectList("reviewlist", vo);  //db와 연결해서 정보를 담아주는 부분
		session.close();
		return rvo;
	...
	public List<PlaceVO> listPlace() {
		SqlSession session = sqlSessionFactory.openSession();
		List<PlaceVO> list = session.selectList("listPlace");
		session.close();// 반납
		return list;
	}
	...
	public List<PlaceVO> getbyaddr1_place_List(PlaceVO vo) {
		...
		System.out.println("batis" + place_List.toString());
		session.close(); // 세션 반납 !반드시 close()해야 오류가 안난다
		return place_List;
	}
	...
	// stayList
	public List<StayVO> listStay() {
		SqlSession session = sqlSessionFactory.openSession();
		List<StayVO> list = session.selectList("listStay");
		session.close();// 반납
		return list;
	}
	public List<PlaceVO> recListPlace() {
		SqlSession session = sqlSessionFactory.openSession();
		List<PlaceVO> list = session.selectList("recListPlace");
		session.close();// 반납
		return list;
	}
